...
namespace prb {
	...
	Vector4f Vector4f::negCol() const {
		return Vector4f(1.f - x, 1.f - y, 1.f - z, w);
	}
	...
	namespace v4u {
		vec4 x(float x) { return { x, 0.f, 0.f, 0.f }; }
		vec4 y(float y) { return { 0.f, y, 0.f, 0.f }; }
		vec4 z(float z) { return { 0.f, 0.f, z, 0.f }; }
		vec4 w(float w) { return { 0.f, 0.f, 0.f, w }; }

		vec4 get(std::stringstream& ss) { return { stru::getf(ss), stru::getf(ss), stru::getf(ss), stru::getf(ss) }; }
		vec4 get(std::ifstream& f)		{ return { stru::getf(f), stru::getf(f), stru::getf(f), stru::getf(f) }; }
	}
