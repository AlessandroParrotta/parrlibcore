...
namespace prb {
	...
	AxisAlignedBoundingBox2D::AxisAlignedBoundingBox2D(std::vector<vec2> const& shape) { 
		for (auto& v : shape) rescale(v);
		align(); 
	}
	...
	AxisAlignedBoundingBox2D AxisAlignedBoundingBox2D::fitted(AxisAlignedBoundingBox2D const& bb) {
		vec2 p = bb.size() / size();			//difference in scale (ratio), x AND y
		float scale = outl::minfabs(p.x, p.y);	//take the smallest one between the two
	...
	AxisAlignedBoundingBox2D AxisAlignedBoundingBox2D::operator+(float const& f) const { return aabb2(vmin + f, vmax + f); }
	AxisAlignedBoundingBox2D AxisAlignedBoundingBox2D::operator-(float const& f) const { return aabb2(vmin - f, vmax - f); }
	AxisAlignedBoundingBox2D AxisAlignedBoundingBox2D::operator*(float const& f) const { return aabb2(vmin * f, vmax * f); }
	AxisAlignedBoundingBox2D AxisAlignedBoundingBox2D::operator/(float const& f) const { return aabb2(vmin / f, vmax / f); }

	AxisAlignedBoundingBox2D AxisAlignedBoundingBox2D::operator+=(float const& f) { vmin += f; vmax += f; return *this; }
	AxisAlignedBoundingBox2D AxisAlignedBoundingBox2D::operator-=(float const& f) { vmin -= f; vmax -= f; return *this; }
	AxisAlignedBoundingBox2D AxisAlignedBoundingBox2D::operator*=(float const& f) { vmin *= f; vmax *= f; return *this; }
	AxisAlignedBoundingBox2D AxisAlignedBoundingBox2D::operator/=(float const& f) { vmin /= f; vmax /= f; return *this; }
	...
	std::vector<vec2> AxisAlignedBoundingBox2D::toVec() const { return { get(0), get(1), get(2), get(3) }; }

	vec2 AxisAlignedBoundingBox2D::edgeCenter(int i) const {
		return ((*this)[i] + (*this)[(i + 1) % 4]) / 2.f;
	}
	vec2 AxisAlignedBoundingBox2D::top() const		{ return edgeCenter(0); }
	vec2 AxisAlignedBoundingBox2D::right() const	{ return edgeCenter(1); }
	vec2 AxisAlignedBoundingBox2D::left() const		{ return edgeCenter(2); }
	vec2 AxisAlignedBoundingBox2D::bottom() const	{ return edgeCenter(3); }
